...
export const getLeagueStandings = async (queryLeagueID = leagueID) => {
	const storedStandings = get(standingsStore)[queryLeagueID];
	if(storedStandings?.standingsInfo) {
		return storedStandings;
	}

	const leagueDataPromise = getLeagueData(queryLeagueID);
	const rostersPromise = getLeagueRosters(queryLeagueID);
	const nflStatePromise = queryLeagueID === leagueID ? getNflState() : Promise.resolve(null);
	...
	const isCurrentNflSeasonLeague = String(nflState?.season) === String(yearData);
	...
	if(
		...
		|| (queryLeagueID === leagueID && isCurrentNflSeasonLeague && nflState?.week < 1)
	) {
		return null;
	}
    ...
    for(const rosterID in rosters) {
	    ...
	    if(divisions) {
	        let week = 0;
	        if(queryLeagueID !== leagueID || leagueData.status === 'complete' || !isCurrentNflSeasonLeague) {
	            ...
	            week = regularSeasonLength + 1;
	        } else if(nflState.season_type == 'complete') {
	            week = regularSeasonLength + 1;
	        }
