...
export const getLeagueTransactions = async (preview, refresh = false) => {
        ...
        if(!refresh && browser) {
                let localTransactions = await JSON.parse(localStorage.getItem("transactions_v2"));
    ...
    if(browser) {
            // update localStorage
        localStorage.setItem("transactions_v2", JSON.stringify(transactionPackage));
...
const checkPreview = (preview, passedTransactions) => {
        if(preview) {
                // If this is being used for a preview component, limit trades and waivers
                const tradesToReturn = 1;
                const waiversToReturn = 3;
